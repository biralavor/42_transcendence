...
export default function FriendsSidebar({ userId, username, currentUser, onViewProfile }) {
  ...
  const [dmOfflineTarget, setDmOfflineTarget] = useState(null)
  ...
  const navigateToWaitingRoom = useCallback((roomId, opponent) => {
    ...
    navigate(`/game/waiting/${roomId}`, {
      state: {
        ...
        opponent: {
          id: hasValidIds ? opponentNumericId : opponent.id,
          username: opponent.username,
          avatarUrl: opponent.avatarUrl || opponent.avatar_url || DEFAULT_AVATAR,
        },
        friendId: hasValidIds ? opponentNumericId : opponent.id,
  ...
  useEffect(() => {
    ...
    const latestRelevant = notifications.find(
      (n) => n.type === 'friend_request' || n.type === 'friend_request_accepted',
    )
  ...
  useEffect(() => {
    ...
    const response = notifications.find(
      (n) =>
        n.type === 'game_invite_response' &&
        !n.read &&
        n.from_user_id === activeInvite.friendId,
    )
  ...
  useEffect(() => {
    ...
    const inviteClient = createGameChannelClient(
      getGameChannelIdForUser(selfId),
      auth.access_token,
      {
        onMessage: (data) => {
          ...
          if (data.type === 'game_invite') {
            ...
            if (isInviteExpired(invite.expiresAt)) return
          ...
          if (data.type === 'game_invite_response') {
            const activeInvite = outgoingInviteRef.current
            if (!activeInvite || data.room_id !== activeInvite.roomId) return
          ...
          if (data.type === 'game_invite_timeout') {
            setIncomingInvite((prev) => {
              if (!prev || prev.roomId !== data.room_id) return prev
  ...
  }, [selfId, auth?.access_token, showInviteToast, resetOutgoingInvite, navigateToWaitingRoom])
  ...
  const handleInviteToGame = async (friend) => {
    const selfNumericId = Number(selfId)
    const friendNumericId = Number(friend.id)

    if (
      !selfUsername ||
      outgoingInviteRef.current ||
      !Number.isInteger(selfNumericId) ||
      !Number.isInteger(friendNumericId)
    ) {
      showInviteToast('Missing player ids for this invite. Reload and try again.', 'danger')
      return
    }

    const roomId = buildInviteRoomId(selfNumericId, friendNumericId)
    const expiresAt = Date.now() + INVITE_TIMEOUT_MS

    const pendingInvite = {
      roomId,
      friendId: friendNumericId,
    ...
    outgoingInviteTimer.current = setTimeout(async () => {
      ...
      try {
        await sendInviteEvent(friendNumericId, {
          type: 'game_invite_timeout',
          room_id: roomId,
          from_user_id: selfNumericId,
          to_user_id: friendNumericId,
        })
      } catch (error) {
        console.error(error)
      }
  ...
  const handleIncomingInviteAccept = async () => {
    ...
    const selfNumericId = Number(selfId)
    const senderId = Number(incomingInvite.fromUserId)

    if (!Number.isInteger(selfNumericId) || !Number.isInteger(senderId)) {
      showInviteToast('Missing player ids for this invite. Reload and try again.', 'danger')
      return
    }
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}))
        console.error('[FriendsSidebar] Accept API error:', {
          status: res.status,
          error: errorData,
        })
  ...
  const handleIncomingInviteDecline = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}))
        console.error('[FriendsSidebar] Decline API error:', {
          status: res.status,
          error: errorData,
        })
  ...
  return (
    <>
      <aside className="friends-sidebar arcade-screen">
        ...
        {inviteToast && (
          <div
            className={`friends-sidebar-alert friends-sidebar-alert-${inviteToast.tone}`}
            role="status"
          >
            {inviteToast.message}
          </div>
            ...
            <ul className="friends-list">
              {requests.map((req) => (
                <li key={req.id} className="friends-list-item friends-request-item">
                  <span className="friends-username">
                    {req.requester_username ?? `Player #${req.requester_id}`}
                  </span>
            ...
            <div className="dm-offline-actions">
              <button
                ...
                onClick={() => {
                  ...
                  clearUnread(slug)
                  navigate(`/chat/${slug}`, {
                    state: { username: selfUsername, userId: selfId },
                  })
                }}
              >
                Open Chat
